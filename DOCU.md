...
##### Building a SequenceModel
A SequenceModel model (_forward_) takes a tokenized input of shape [B, L] and LongTensor dtype.
...
If only these 3 values are specified, SequenceModel will construct a transformer, usinc MultiHeadAttentionMixer
and MLP.
To easily construct and test a Custom SequenceModel, its enough to specify a channel mixer constructor, a spatial mixer constructor,
and then pass the SequenceModel, the constructors arguments by (spatial/channel)_args={"arg1":1, "arg2:-, ..."}.
...
```
Additionally, _embedder_ and _pos_encoder_ classes can be also passes to SequenceModel init. pos_encoder can be also set to None -> Identity, False -> Identity, True -> Sinusoidal
