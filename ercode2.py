...
class Hyperparameters:
    ...
    warmdown_iters = int(os.environ.get("WARMDOWN_ITERS", 126))
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor]):
    ...
    stats = dict.fromkeys(
        ("param_count", "num_tensors", "num_float_tensors", "num_nonfloat_tensors", "baseline_tensor_bytes",
         "int8_payload_bytes"),
        0,
    )
...
class Block(nn.Module):
    def __init__(
            ...
            rope_base: float,
            qk_gain_init: float,
            num_skips: int = 1,  # Injected from Snippet 1
            layer_idx: int = 0,
            ln_scale: bool = False,
            dtg: bool = False,
            gated_attention: bool = False,
    ...
    def forward(
        ...
        # Scalar broadcasting handles the shape differences automatically
        m_x = mix[0]
        m_x0 = mix[1]
...
class GPT(nn.Module):
    def __init__(
            ...
            rope_base: float,
            qk_gain_init: float,
            num_projections: int = 0,  # Injected
            mtp_num_heads: int = 0,
        ...
        self.num_projections = num_projections  # Injected
        self.model_dim = model_dim

        self.tok_emb = nn.Embedding(vocab_size, model_dim * (1 + num_projections))
        ...
        # My secondary embedding thing
        self.se_gate = nn.Parameter(torch.Tensor([0.0]))
        self.mid_loss_weight = 1 / 3
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    rope_base,
                    qk_gain_init,
                    num_skips=1 + num_projections,  # Injected logic
                    layer_idx=i,
                    ln_scale=ln_scale,
                    dtg=dtg,
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        n = self.num_layers
        chunks = self.tok_emb(input_ids).chunk(1 + self.num_projections, dim=-1)
        x = chunks[0]
        xss = tuple(chunks[1:]) if self.num_projections > 0 else None
        ...
        for i in range(self.num_encoder_layers):
            ...
            use_xss = None # xss if i > 4 else None
            x, raw_v = self.blocks[i](
                x, x0, xss=use_xss,
                q_w=self.qo_bank[i], k_w=self.kv_bank[i], v_w=self.kv_bank[n + i],
                out_w=self.qo_bank[n + i], up_w=self.mlp_up_bank[i], down_w=self.mlp_down_bank[i],
                v_embed=ve, v0=v0
            )
        ...
        z = F.linear(F.rms_norm(x), self.tok_emb.weight[:, self.model_dim:])
        z_flat = z.reshape(-1, z.size(-1))
        mid_loss = F.cross_entropy(z_flat.float(), targets, reduction="mean")
        x = x + self.se_gate * (torch.softmax(z, dim=-1) @ self.tok_emb.weight[:, :self.model_dim])
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](
                ...
                out_w=self.qo_bank[n + bi], up_w=self.mlp_up_bank[bi], down_w=self.mlp_down_bank[bi],
                v_embed=ve, v0=v0
            )
        ...
        if self.tie_embeddings:
            tied_weights = self.tok_emb.weight[:, :self.model_dim]
            logits_proj = F.linear(x_flat, tied_weights)
        else:
        ...
        main_loss = main_loss + mid_loss * self.mid_loss_weight
        return main_loss
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        if self.tie_embeddings:
            logits_proj = F.linear(x_flat, self.tok_emb.weight[:, :self.model_dim])
        else:
            if self.lm_head is None:
                raise RuntimeError("lm_head is required when tie_embeddings=False")
            logits_proj = self.lm_head(x_flat)
    ...
    for ci in range(num_chunks):
        ...
        if rank == 0 and (ci % 10 == 0 or ci == num_chunks - 1):
            ...
            rbpb = rl / math.log(2.0) * (
                    token_count.item() / max(byte_count.item(), 1)) if token_count.item() > 0 else 0.0
...
def main() -> None:
    ...
    def lr_mul(step: int, elapsed_ms: float) -> float:
        ...
        if max_wallclock_ms is None:
            ...
            return max((args.iterations - step) / max(args.warmdown_iters, 1),
                       0.0) if warmdown_start <= step < args.iterations else 1.0
    ...
    ema_state = {name: t.detach().float().clone() for name, t in
                 base_model.state_dict().items()} if args.ema_enabled else {}
    ...
    while True:
        ...
        if args.late_qat and args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
